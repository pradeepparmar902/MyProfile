...
export async function PUT(request, { params }) {
  ...
  const existing = await db.professionSelf.findFirst({ where: { id, userId: user.id } });
  if (!existing) return error("Self Business / Training record not found.", 404);
...
export async function DELETE(_request, { params }) {
  ...
  const existing = await db.professionSelf.findFirst({ where: { id, userId: user.id } });
  if (!existing) return error("Self Business / Training record not found.", 404);
  await db.professionSelf.delete({ where: { id } });
  return json({ ok: true });
}
