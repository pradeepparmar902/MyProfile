...
function MindmapCanvas({ roadmap, onClose, onSave, onDelete }) {
  ...
  const [titleValue, setTitleValue] = useState(roadmap.title || 'Untitled Roadmap');
  const reactFlowWrapper = useRef(null);

  // Initialize nodes and edges from roadmap.mindmapData or use defaults
  const initialData = roadmap.mindmapData ? (typeof roadmap.mindmapData === 'string' ? JSON.parse(roadmap.mindmapData) : roadmap.mindmapData) : null;
    ...
    initialData?.nodes || [
      {
        ...
        data: { label: roadmap.title, isRoot: true, color: '#3b82f6', status: 'In Progress', notes: '' },
      },
    ]
  );
  ...
  const handleSave = async () => {
    ...
    if (roadmap.id.startsWith('new')) {
      ...
      if (data.roadmap) {
        roadmap.id = data.roadmap.id; // Update ID so future saves are PUTs
      }
    } else {
    ...
    setIsSaving(false);
    if (onSave) onSave(mindmapData, roadmap.id.startsWith('new'), titleValue);
  };
  ...
  return (
    <div className={`relative ${roadmap.isInline ? "w-full h-full flex flex-col rounded-2xl overflow-hidden shadow-sm border border-slate-200" : "fixed inset-0 z-50 flex flex-col bg-slate-900/95 backdrop-blur-sm"} print:static print:overflow-visible print:bg-white`} onKeyDown={onKeyDown}>
...
export default function RoadmapMindmap({ roadmap, onClose, onSave, onDelete }) {
  if (!roadmap) return null;
  return (
    <ReactFlowProvider>
      <MindmapCanvas roadmap={roadmap} onClose={onClose} onSave={onSave} onDelete={onDelete} />
    </ReactFlowProvider>
  );
}
